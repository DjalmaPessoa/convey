...
for ( this.epsilon in c(.5,1,2) ) {
  ...
  # return test context
  context( paste( "svyatk epsilon =" , this.epsilon , "output survey.design and svyrep.design" ) )
  ...
  test_that("svyatk works on unweighted designs", {
    expect_false( is.na ( coef( svyatk( ~api00, design=dstrat1 , epsilon = this.epsilon ) ) ) )
    expect_false( is.na ( SE( svyatk( ~api00, design=dstrat1 , epsilon = this.epsilon ) ) ) )
  } )
  ...
  # calculate estimates
  a1 <- svyatk( ~eqincome , des_eusilc , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
  a2 <- svyby( ~eqincome , ~hsize, des_eusilc, svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
  a2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc, svyatk , epsilon = this.epsilon , deff = TRUE , covmat = FALSE )
  b1 <- svyatk( ~eqincome , des_eusilc_rep , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
  b2 <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
  b2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyatk , epsilon = this.epsilon , deff = TRUE , covmat = FALSE )
  ...
  test_that("database svyatk",{
    ...
    # calculate estimates
    c1 <- svyatk( ~eqincome , dbd_eusilc , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
    c2 <- svyby( ~eqincome , ~hsize, dbd_eusilc, svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
  ...
  # calculate estimates
  sub_des <- svyatk( ~eqincome , design = subset( des_eusilc , hsize == 1) , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
  sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
  sub_rep <- svyatk( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
  sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
  ...
  test_that("dbi subsets equal non-dbi subsets",{
    ...
    # calculate estimates
    sub_dbd <- svyatk( ~eqincome , design = subset( dbd_eusilc     , hsize == 1) , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
    sub_dbr <- svyatk( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) , epsilon = this.epsilon , deff = TRUE , linearized = TRUE )
    sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc     , FUN = svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
    sby_dbr <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc_rep , FUN = svyatk , epsilon = this.epsilon , deff = TRUE , covmat = TRUE )
