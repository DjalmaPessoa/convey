...
test_that("svyjdiv works on unweighted designs", {
  expect_false( is.na ( coef( svyjdiv( ~api00, design=dstrat1 , epsilon = this.epsilon ) ) ) )
...
# calculate estimates
a1 <- svyjdiv( ~eqincome , des_eusilc , deff = TRUE , linearized = TRUE )
a2 <- svyby( ~eqincome , ~hsize, des_eusilc, svyjdiv , deff = TRUE , covmat = TRUE )
a2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc, svyjdiv , deff = TRUE , covmat = FALSE )
b1 <- svyjdiv( ~eqincome , des_eusilc_rep , deff = TRUE , linearized = TRUE )
b2 <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyjdiv , deff = TRUE , covmat = TRUE )
b2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyjdiv , deff = TRUE , covmat = FALSE )
d1 <- svygei( ~eqincome , des_eusilc, epsilon = 0, deff = TRUE , linearized = TRUE )
e1 <- svygei( ~eqincome , des_eusilc, epsilon = 1, deff = TRUE , linearized = TRUE )
...
test_that( "output svyjdiv" , {
  ...
  expect_equal( as.numeric( attr( a1 , "linearized" ) ) , as.numeric( rowSums( sapply( list( d1 , e1 ) , attr , "linearized" ) ) ) )
...
test_that("database svyjdiv",{
  ...
  # calculate estimates
  c1 <- svyjdiv( ~eqincome , dbd_eusilc , deff = TRUE , linearized = TRUE )
  c2 <- svyby( ~eqincome , ~hsize, dbd_eusilc, svyjdiv , deff = TRUE , covmat = TRUE )
...
# calculate estimates
sub_des <- svyjdiv( ~eqincome , design = subset( des_eusilc , hsize == 1) , deff = TRUE , linearized = TRUE )
sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyjdiv , deff = TRUE , covmat = TRUE )
sub_rep <- svyjdiv( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , deff = TRUE , linearized = TRUE )
sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyjdiv , deff = TRUE , covmat = TRUE )
...
test_that("dbi subsets equal non-dbi subsets",{
  ...
  # calculate estimates
  sub_dbd <- svyjdiv( ~eqincome , design = subset( dbd_eusilc     , hsize == 1) , deff = TRUE , linearized = TRUE )
  sub_dbr <- svyjdiv( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) , deff = TRUE , linearized = TRUE )
  sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc     , FUN = svyjdiv , deff = TRUE , covmat = TRUE )
  sby_dbr <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc_rep , FUN = svyjdiv , deff = TRUE , covmat = TRUE )
