...
test_that("svyiqalpha works on unweighted designs", {
  expect_false( is.na ( coef( svyiqalpha( ~api00, design=dstrat1 ) ) ) )
  expect_false( is.na ( SE( svyiqalpha( ~api00, design=dstrat1 ) ) ) )
} )
...
# calculate estimates
a1 <- svyiqalpha( ~eqincome , des_eusilc , deff = TRUE , linearized = TRUE )
a2 <- svyby( ~eqincome , ~hsize, des_eusilc, svyiqalpha , deff = TRUE , covmat = TRUE )
a2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc, svyiqalpha , deff = TRUE , covmat = FALSE )
b1 <- svyiqalpha( ~eqincome , des_eusilc_rep , deff = TRUE , linearized = TRUE )
b2 <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyiqalpha , deff = TRUE , covmat = TRUE )
b2.nocov <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyiqalpha , deff = TRUE , covmat = FALSE )
...
test_that("database svyiqalpha",{
  ...
  # calculate estimates
  c1 <- svyiqalpha( ~ eqincome , dbd_eusilc , deff = TRUE , linearized = TRUE )
  c2 <- svyby( ~ eqincome , ~hsize , dbd_eusilc , FUN = svyiqalpha , deff = TRUE , covmat = TRUE )
...
# calculate estimates
sub_des <- svyiqalpha( ~eqincome , design = subset( des_eusilc , hsize == 1) , deff = TRUE , linearized = TRUE )
sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyiqalpha , deff = TRUE , covmat = TRUE )
sub_rep <- svyiqalpha( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , deff = TRUE , linearized = TRUE )
sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyiqalpha , deff = TRUE , covmat = TRUE )
...
test_that("dbi subsets equal non-dbi subsets",{
  ...
  # calculate estimates
  sub_dbd <- svyiqalpha( ~eqincome , design = subset( des_eusilc , hsize == 1) , deff = TRUE , linearized = TRUE )
  sby_dbd <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyiqalpha , deff = TRUE , covmat = TRUE )
  sub_dbr <- svyiqalpha( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , deff = TRUE , linearized = TRUE )
  sby_dbr <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyiqalpha , deff = TRUE , covmat = TRUE )
